...
export function Sidebar({ displayName, initials }: { displayName: string; initials: string }) {
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-[240px] bg-white border-r border-slate-200 flex flex-col z-40">
      ...
      <div className="p-6 border-b border-slate-100">
        <Link href="/snapshot" className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-slate-800">CFO Buddy</h1>
      ...
      <nav className="flex-1 py-6 px-3 space-y-1">
        <p className="px-3 mb-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Main Menu</p>
        {NAV_ITEMS.map((item) => {
          const isActive = pathname === item.href || (item.href !== "/snapshot" && pathname.startsWith(item.href));
          return (
            <Link
              key={item.href}
              href={item.href}
